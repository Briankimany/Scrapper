...
from .master import (convert_duration_format , 
                    generate_random_chars ,
                      get_file_size , 
                      change_str_deltatime , 
                      save_load_program_data , 
                      get_link_default_state)
...
class DataBase:
    """
    Attributes:
        parent_dir (Path): Path where files will be downloaded.
        all_links (list): Batched FormatLink objects.
        data (list): Data source from the SQL database.

    Methods:
        download(): Triggers download of all links in batches.
        inspect(): Prints a summary of the file names in batches.
    """

    def __init__(self, parent_dir, session, link_model, batch_size=3, max_num=2000):
        self.parent_dir = Path(parent_dir)
        self.all_links = []

        query = session.query(link_model).limit(max_num)
        data = [(row.url, row.name, row.duration) for row in query.all()]

        self.data = data
        batched_data = list(more_itertools.batched(self.data, batch_size))

        with tqdm(total=len(self.data), desc="Processing") as pbar:
    ...
    def initialize_link(self, i):
        return FormatLink(link_data=i, parent_dir=self.parent_dir, in_data_base=True)
